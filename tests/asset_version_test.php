...
try {
    ...
    if ((function_exists('posix_geteuid') && posix_geteuid() === 0) || PHP_OS_FAMILY === 'Windows') {
        av_ok(true, 'AV-21', PHP_OS_FAMILY === 'Windows' ? 'ilegible omitido (Windows)' : 'ilegible omitido (proceso root)');
        av_ok($warned === false, 'AV-22', PHP_OS_FAMILY === 'Windows' ? 'ilegible sin warning (Windows)' : 'ilegible sin warning (root)');
    } else {
